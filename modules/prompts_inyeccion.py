    ...
    def _inyectar_formato_z_image(self, modelo: str, specs: dict, extra: str) -> str:
        ...
        if lora_trigger:
            bloques_positivos = (
                ...
                "etc) + ANGLE (low, eye-level, overhead) + POSE/action + "
                "composition (rule of thirds, centered, leading lines). "
                "⚠️ DO NOT describe physical traits of the subject (face, "
                "hair color, eye color, skin texture, ethnicity, age, body "
                "type) — the LoRA weights already encode that. Only mention "
                "wardrobe IF it's specifically required by the scene and "
                "not contradicted by the LoRA. Keep this block focused on "
                "FRAMING, GESTURE, and the SCENE around the subject.>\n"
                ...
                f"'liminal horror atmosphere'). NO physical description of "
                f"the subject here either.>\n"
            ...
            nota_lora = (
                ...
                f"\n"
                f"🎭 EL LORA YA APORTA LOS RASGOS DEL PERSONAJE/ESTILO:\n"
                f"  • Los pesos del LoRA ya tienen la apariencia entrenada "
                f"(cara, pelo, ojos, complexión, marca de estilo).\n"
                f"  • ❌ NO inventes ni describas rasgos físicos del sujeto "
                f"en [Subject & Composition]. Si la idea del usuario NO "
                f"especifica rasgos físicos, NO los añadas tú.\n"
                f"  • ✅ Centra [Subject & Composition] en: plano (close-up/"
                f"medium/wide), ángulo de cámara, pose/acción, composición "
                f"y elementos de la escena alrededor del sujeto.\n"
                f"  • Si el usuario SÍ especifica rasgos en la idea, "
                f"respétalos literalmente (no los expandas).\n"
            )
        else:
            bloques_positivos = (
