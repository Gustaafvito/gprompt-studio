"""Tests para WorkersIaService (modules/workers_ia.py).
...
Cobertura de los 5 workers que se ejecutan en threads. Mockeamos:
  • app.after(0, fn) → fn() inmediato (sin event loop tk)
  • app.deepseek.generar → respuesta fija
  • app.vision.describir → tupla (desc, motor) fija
...
def _host(*, deepseek_resp="POSITIVE PROMPT: nice\nNEGATIVE PROMPT: bad",
          ...
          **overrides):
    """Construye un WorkersIaService(app) con app simulado.

    h.app tiene todos los mocks; h.app._worker_* son los métodos del service.
    """
    app = SimpleNamespace()
    defaults = dict(
        # Datos
    ...
    for k, v in defaults.items():
        setattr(app, k, v)
    # _worker_ia llama a self.app.refinar.mostrar_diff_refinamiento en
    # modo refinamiento. Apuntamos al mismo mock que
    # _mostrar_diff_refinamiento para mantener las aserciones de tests.
...
class TestWorkerIa:
    ...
    def test_caso_basico_actualiza_salida_y_historial(self):
        ...
        h.app.guardar_en_historial.assert_called_once()
        h.app.actualizar_salida.assert_called_once()
        h.app.toggle_botones.assert_called_with(True)
        h.app._detener_progreso.assert_called_once()
    ...
    def test_modo_refinamiento_abre_diff_modal(self):
        ...
        h.app._mostrar_diff_refinamiento.assert_called_once()
        args = h.app._mostrar_diff_refinamiento.call_args[0]
        assert args[0] == "viejo prompt"
        # actualizar_salida NO debe llamarse en modo refinamiento
        h.app.actualizar_salida.assert_not_called()

    def test_modo_ideas_invoca_mostrar_ideas(self):
        h = _host(deepseek_resp="1. idea uno\n2. idea dos\n3. idea tres")
        h._worker_ia("p", es_ideas=True)
        h.app._mostrar_ideas.assert_called_once()
        ideas = h.app._mostrar_ideas.call_args[0][0]
        assert len(ideas) >= 1
    ...
    def test_modo_variaciones_invoca_mostrar_variaciones(self):
        ...
        h.app._mostrar_variaciones.assert_called_once()
        h.app.actualizar_salida.assert_not_called()
    ...
    def test_max_tokens_grande_si_specs_max_chars_grande(self, monkeypatch):
        ...
        h.app.deepseek.generar = lambda *a, **k: (
...
class TestWorkerVision:
    ...
    def test_pone_descripcion_en_idea_si_estaba_vacia(self):
        h = _host()
        h._worker_vision()
        h.app.txt_idea.delete.assert_called_with("1.0", "end")
        h.app.txt_idea.insert.assert_called_with("1.0", "una descripción visual")
        h.app.toggle_botones.assert_called_with(True)

    def test_concatena_con_idea_previa_si_existe(self):
        h = _host(txt_idea=_txt("mi idea previa"))
        h._worker_vision()
        idea_insertada = h.app.txt_idea.insert.call_args[0][1]
    ...
    def test_excepcion_muestra_error_y_rehabilita_botones(self):
        h = _host()
        h.app.vision = SimpleNamespace(describir=MagicMock(side_effect=Exception("boom")))
        h._worker_vision()
        msgs = [c.args[0] for c in h.app.set_estado.call_args_list]
...
class TestWorkerPromptQuick:
    ...
    def test_construye_peticion_quick_mode(self):
        capturado = {}
        h = _host()
        h.app.deepseek.generar = lambda p, **k: (
    ...
    def test_quick_quita_negative_si_modelo_no_lo_soporta(self):
        ...
        h.app.deepseek.generar = lambda p, **k: "POSITIVE PROMPT: a\nNEGATIVE PROMPT: b"
        h._worker_prompt_quick("idea")
        texto = h.app.actualizar_salida.call_args[0][0]
        assert "NEGATIVE" not in texto.upper()
...
class TestWorkerImagenAPrompt:
    ...
    def test_actualiza_anclaje_visual_con_descripcion(self):
        h = _host()
        h._worker_imagen_a_prompt()
        assert h.app._ultimo_anclaje_visual == "una descripción visual"

    def test_excepcion_muestra_error(self):
        h = _host()
        h.app.vision = SimpleNamespace(describir=MagicMock(side_effect=Exception("vision_err")))
        h._worker_imagen_a_prompt()
        msgs = [c.args[0] for c in h.app.set_estado.call_args_list]
        assert any("Error" in m for m in msgs)
    ...
    def test_modo_referencia_visual_usa_prompt_distinto(self):
        ...
        capturado = {}
        h = _host()
        h.app.switch_ref_visual_var = _var(True)
        h.app.deepseek.generar = lambda p, **k: (
